...
public class WordDuoList {
    ...
    public WordDuoList(String[] words){
        ...
        for (int i = 0; i < len; i++){
            for (int j = i+1; j < len; j++){
                WordDuo pair = new WordDuo (words[i], words[j]);
                allDuos.add(pair);
            }
        }
    }
    ...
    public int numMatches(){
        int result = 0;
        for (WordDuo tester : allDuos) {
            if (tester.getFirst().equals(tester.getSecond())) {
                result++;
            }
        }
        return result;
    ...
    public void moveMatchesToTop(){
        ...
        for (int i = 0; i < allDuos.size(); i++){
            WordDuo tester = allDuos.get(i);
            if (tester.getFirst().equals(tester.getSecond())){
                allDuos.remove(i);
                allDuos.add(0, tester);
                i--;
            }
        }
    }
